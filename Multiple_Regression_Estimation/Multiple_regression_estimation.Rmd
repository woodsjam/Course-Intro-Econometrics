...
$$\max_{\hat{\beta}}(y-\hat{\beta}x)'(y-\hat{\beta}x)$$

Split this out so it is easier to work with and take the derivative.

$$\max_{\hat{\beta}} y'y - 2\hat{\beta}x'y +\hat{\beta}^2 x'x\\
...
We know from the reading that $R^2$ will increase when you add more variables. Lets take a look at what happens when you include variables that are not part of the population relationship. In other words, you started making a kitchen sink model.

Lets start with a regression where $x_1$ and $x_2$ are used to explain $y$ but we are going to include some polynomial terms for $x_2$.

The true population relationship is:

$$y = \alpha + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_2^2 + \epsilon$$

With $\alpha =1$, $\beta_1 =2$, $\beta_2 = 3$ and $\beta_3 = -.2$. We will start with this specification and then slowly add the irrelevant variables, additional powers of $x_2$. These new variables will be correlated with the other powers of $x_2$

```{r}

irrelevant_variable_data <- data.frame(
  err = rnorm(200, sd = 3),
  x1 = rnorm(200, mean = 1),
  x2 = rnorm(200, mean = 2)
) %>%
  mutate(y = 1 + 2 * x1 + 3 * x2 - .2 * (x2^2) + err)
...
#       sidebarPanel(
# 
#         sliderInput("Poly", "Degree Polynomial", min = 1, max = 20, value = 2, step = 1 )
#       ),
# 
#       mainPanel(
#         verbatimTextOutput("lm_result"),
#         verbatiimTextOutput("corr_x2_polynomials")
#       )
#     )
#   ),
# 
...
# 
# output$lm_result = renderPrint({
#       irrelevant_variable_data %>%
#         lm(y ~ x1 + poly(x2,input$Poly, raw = TRUE), data = .) %>%
#         summary()
#     }),
# 
# output$corr_x2_polynomials = renderPrint({
#   cor(poly(irrelevant_variable_data$x2,input$Poly, raw = TRUE)) 
# })
#   }
```
